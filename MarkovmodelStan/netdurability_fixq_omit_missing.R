...
if(is.null(counts)){
  ...
  counts <- matrix(data=0,nrow=5,ncol=5)
  source("./net_load.R")
  ...
  counts <- matrix(as.integer(counts),nrow=5,ncol=5)  
  counts <- counts[,1:5]
}

###################
#models
netdurability_fixq_omit_missing <- "./netdurability_fixq_omit_missing.stan"
...
# fit
fit_posterior<-stan(
  file = netdurability_fixq_omit_missing,  # Stan program
