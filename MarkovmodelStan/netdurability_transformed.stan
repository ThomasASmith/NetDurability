// Stan model for net durability surveys with q and p0 fixed 
data{
...
parameters{
  ...
  real loga2; // see diagram
  real loga3; // see diagram
  real loga4; // see diagram
...
transformed parameters{
  real a2 = exp(loga2); // see diagram
  real a3 = exp(loga3); // see diagram
  real a4 = exp(loga4); // see diagram
  real<lower=0> v3 = 2*exp(logavg1)/(exp(lograt1)+1); // see diagram
  real<lower=0> u1 = exp(lograt1)*v3; // see diagram
  real<lower=0> u2 = 2*exp(logavg2)/(exp(lograt2)+1); // see diagram
  real<lower=0> v4 = exp(lograt2)*u2; // see diagram
  // declare
